# 末日ESG小尖兵

一個刺激的2.5D網頁遊戲，玩家作為ESG小尖兵在末日環境中冒險！

## 🎮 遊戲特色

### 核心玩法
- **多關卡系統**：4個不同難度的關卡，逐步解鎖
- **安全區域系統**：4x4格的安全起始區域，怪物無法進入
- **遠距離攻擊**：300像素攻擊範圍，每0.3秒自動攻擊
- **三種怪物類型**：普通怪物隨機移動，追蹤怪物會追擊玩家，砲塔怪物遠程攻擊
- **擊殺計數系統**：記錄消滅的怪物數量

### 新增功能
- **音效系統**：完整的背景音樂和音效，可獨立控制
- **粒子效果**：攻擊、擊中、爆炸等視覺特效
- **遊戲統計**：詳細的遊戲數據追蹤
- **進度保存**：自動保存關卡進度
- **道具系統**：四種不同道具，地圖掉落和怪物掉落
- **通關條件**：必須收集足夠道具才能通關

### 遊戲機制
- **自動攻擊**：停止移動時自動發射黃色彈幕
- **怪物血量**：每隻怪物需要攻擊2次才會消失
- **追蹤範圍**：追蹤怪物只在500像素內追擊玩家
- **通關條件**：收集足夠數量的道具後，找到地圖中心的出口並按空白鍵

## 🎯 操作方式

| 按鍵 | 功能 |
|------|------|
| 方向鍵 | 移動角色 |
| 空白鍵 | 執行動作（打招呼/通關） |
| 音效按鈕 | 控制背景音樂和音效開關 |

## 🏆 遊戲目標

收集關卡要求的道具數量，在時間限制內找到地圖中心的出口並通關！

## 🎨 視覺特色

- **2.5D視角**：使用HTML5 Canvas繪製
- **精美地圖**：每個關卡都有獨特的地圖瓦片
- **網格系統**：100像素間距的網格線幫助定位
- **安全區域**：淡藍色邊框標示4x4安全區域
- **怪物區分**：
  - 紅色方塊：普通怪物（隨機移動）
  - 粉紅色方塊：追蹤怪物（會追擊玩家）
  - 深紅色方塊：砲塔怪物（遠程攻擊）
- **攻擊特效**：黃色小圓點彈幕
- **粒子效果**：攻擊軌跡、擊中爆炸、受傷特效
- **計時器**：右上角顯示剩餘時間，低於5秒變黃色，低於3秒變紅色
- **道具系統**：
  - OPEE（紅色）：地圖道具，第一關出現
  - 共感頻率器（綠色）：地圖道具，第三關出現
  - 綠能聚合晶體（橙色）：怪物掉落，追蹤怪物掉落
  - 動能核心（藍色）：怪物掉落，砲塔怪物掉落
- **出口狀態**：綠色表示可通關，紅色表示條件未滿足

## 🎲 遊戲設定

### 關卡設定
- **關卡數量**：4個關卡，逐步解鎖
- **地圖大小**：隨關卡遞增（2400x1800 到 4800x3600像素）
- **可視範圍**：800x600像素
- **安全區域**：4x4格（200x200像素）

### 怪物設定
- **普通怪物**：30-60隻（隨關卡增加）
- **追蹤怪物**：10-25隻（隨關卡增加）
- **砲塔怪物**：2-5隻（隨關卡增加）
- **怪物血量**：每隻2-5點血量
- **怪物大小**：60x60像素

### 攻擊設定
- **攻擊範圍**：300像素
- **攻擊頻率**：每0.3秒一次
- **彈幕速度**：每秒8像素
- **彈幕大小**：4像素半徑

### 時間設定
- **遊戲時間**：90-180秒（隨關卡增加）
- **倒數計時**：實時顯示剩餘時間

## 🚀 技術實現

### 技術棧
- **前端**：純HTML5 Canvas
- **語言**：JavaScript
- **渲染**：requestAnimationFrame
- **碰撞檢測**：矩形碰撞檢測
- **路徑計算**：歐幾里得距離算法
- **音效系統**：HTML5 Audio API
- **粒子系統**：自定義粒子引擎

### 核心功能
- **遊戲迴圈**：60FPS流暢運行
- **鍵盤輸入**：實時響應方向鍵和空白鍵
- **鏡頭跟隨**：玩家始終在畫面中央
- **怪物AI**：隨機移動、追蹤和遠程攻擊邏輯
- **計時系統**：精確的倒數計時
- **音效管理**：背景音樂和音效控制
- **粒子效果**：動態粒子系統
- **進度保存**：Cookie-based進度保存

## 🎯 遊戲策略

### 初學者策略
1. 在安全區域內觀察怪物分布
2. 利用遠距離攻擊消滅追蹤怪物
3. 避開怪物密集區域
4. 快速尋找地圖邊緣的綠色出口

### 進階策略
1. 優先消滅追蹤怪物
2. 利用安全區域作為避難所
3. 規劃最佳路徑到出口
4. 在時間緊迫時冒險衝刺

## 🏁 勝利條件

- 收集關卡要求的道具數量
- 在時間限制內找到地圖中心的出口
- 站在出口上按空白鍵
- 顯示「恭喜通關！」畫面

### 道具通關條件
- **第1關**：收集1個OPEE（地圖道具）
- **第2關**：收集10個綠能聚合晶體（怪物掉落）
- **第3關**：收集15個共感頻率器 + 5個綠能聚合晶體
- **第4關**：收集1個動能核心（砲塔怪物掉落）

## 💀 失敗條件

- 被怪物觸碰到
- 60秒時間到未通關

## 🔄 重新開始

- 遊戲結束或通關後按空白鍵
- 重置所有遊戲狀態
- 重新生成怪物和出口
- 計時器和擊殺數歸零

## 📊 遊戲統計

- **擊殺計數**：右上角顯示消滅的怪物數量
- **時間顯示**：實時倒數計時
- **Console記錄**：每次擊殺都會在瀏覽器控制台顯示

## 🎮 遊戲體驗

### 緊張刺激
- 60秒時間限制增加緊迫感
- 追蹤怪物帶來持續威脅
- 倒數計時顏色變化提醒時間緊迫

### 策略性
- 安全區域提供戰術選擇
- 遠距離攻擊需要精準判斷
- 路徑規劃影響通關效率

### 成就感
- 擊殺計數器記錄表現
- 快速通關挑戰
- 高擊殺數成就

## 🛠️ 開發特色

- **模組化設計**：功能分離，易於維護
- **效能優化**：只渲染可視範圍內的物件
- **響應式設計**：適應不同螢幕尺寸
- **錯誤處理**：穩定的遊戲運行


---

**享受這個刺激的逃脫挑戰吧！** 🎮✨ 
